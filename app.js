const DB_KEY = "reyed_demo_db_v3";
const SESSION_KEY = "reyed_session_v3";
...
function rideCard(ride,options={}){
  ...
  return `<article class="ride-card ${!["completed","cancelled"].includes(ride.status)?"active":""}">
    ...
    <div class="small">Rider: ${escapeHTML(rider?.name||"Unknown")} · Driver: ${escapeHTML(driver?.name||"Not assigned")} · ${dateText(ride.createdAt)}</div>
